...
public class TileEntityAlloyMixer extends TileEntityFoundryPowered {
	protected class FluidHandler implements IFluidHandler {
		...
		public int fill(FluidStack resource, boolean doFill) {
			...
			if (partial != -1) { return fillTank(partial, resource, doFill); }
			if (empty != -1) { return fillTank(empty, resource, doFill); }
			return 0;
		}
	...
	private void mixAlloy() {
		...
		int energy_used = 0;
		while (true) {
			if (energy_used >= 2500) { return; }
			if (!recipe.matchesRecipe(input_tank_fluids, recipe_order)) { return; }
			FluidStack output = recipe.getOutput();

			if (tanks[TANK_OUTPUT].fill(output, false) < output.amount) { return; }
			int required_energy = 10 * output.amount;
			if (useFoundryEnergy(required_energy, false) < required_energy) { return; }
