...
public class RFCModel implements IModel {
	static private final class BakedItemModel implements IBakedModel {
		private final class RFCOverride extends ItemOverrideList {
			...
			public IBakedModel handleItemState(IBakedModel originalModel, ItemStack stack, World world, EntityLivingBase entity) {
				...
				if (fluid == null || fluid.amount == 0) { return originalModel; }
				int y = fluid.amount * 10 / Fluid.BUCKET_VOLUME;
				if (y <= 0) { return originalModel; }
				...
				builder.addAll(getQuadsForSpriteSlice(1, bakedTextureGetter.apply(texture), format, 4, 3, 12, 3 + y, fluid.getFluid().getColor(), transform));
	...
	public static ImmutableList<BakedQuad> getQuadsForSprite(int tint, TextureAtlasSprite sprite, VertexFormat format, Optional<TRSRTransformation> transform) {
		...
		builder.add(buildQuad(format, transform, EnumFacing.NORTH, sprite, tint, 0, 0, 7.5f / 16f - 0.0002f * tint, sprite.getMinU(), sprite.getMaxV(), 0, 1, 7.5f / 16f - 0.0002f * tint, sprite.getMinU(), sprite.getMinV(), 1, 1, 7.5f / 16f - 0.0002f * tint, sprite.getMaxU(), sprite.getMinV(), 1, 0, 7.5f / 16f - 0.0002f * tint, sprite.getMaxU(), sprite.getMaxV(), 0xFFFFFFFF));
		builder.add(buildQuad(format, transform, EnumFacing.SOUTH, sprite, tint, 0, 0, 8.5f / 16f + 0.0002f * tint, sprite.getMinU(), sprite.getMaxV(), 1, 0, 8.5f / 16f + 0.0002f * tint, sprite.getMaxU(), sprite.getMaxV(), 1, 1, 8.5f / 16f + 0.0002f * tint, sprite.getMaxU(), sprite.getMinV(), 0, 1, 8.5f / 16f + 0.0002f * tint, sprite.getMinU(), sprite.getMinV(), 0xFFFFFFFF));
		return builder.build();
	}
	...
	static public ImmutableList<BakedQuad> getQuadsForSpriteSlice(int tint, TextureAtlasSprite sprite, VertexFormat format, int min_x, int min_y, int max_x, int max_y, int color, Optional<TRSRTransformation> transform) {
		...
		builder.add(buildQuad(format, transform, EnumFacing.NORTH, sprite, tint, x1, y1, z1, min_u, max_v, x1, y2, z1, min_u, min_v, x2, y2, z1, max_u, min_v, x2, y1, z1, max_u, max_v, color));

		builder.add(buildQuad(format, transform, EnumFacing.SOUTH, sprite, tint, x1, y1, z2, min_u, max_v, x2, y1, z2, max_u, max_v, x2, y2, z2, max_u, min_v, x1, y2, z2, min_u, min_v, color));
